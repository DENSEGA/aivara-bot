...
params=[['Параметр','Значение'],['Тип','${esc(data.houseType||'-')}'],['Площадь','${esc(String(data.area||'-'))} м²'],['Этажность','${esc(data.floors||'-')}'],['Стиль','${esc(data.style||'-')}'],['Спален','${esc(String(data.bedrooms||'-'))}'],['Утепление стен','${esc(data.wallInsulation||'-')}'],['Крыша','${esc((data.roofType||'')+', '+(data.roofMaterial||''))}'],['Фасад','${esc(data.facade||'-')}'],['Окна','${esc(data.windows||'-')}'],['Входная дверь','${esc(data.door||'-')}']]
