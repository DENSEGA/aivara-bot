...
async function chat(messages) {
  const systemMessage = {
    role: 'system',
    content: `
Ты - профессиональный помощник компании ЭкоКаркас.
Отвечай четко, уверенно и по делу.
Не пиши, что ты обучен на данных до какого-то года.
Не пиши, что не знаешь свою версию.
Пиши естественно, современно и грамотно на русском языке.
Если вопрос про дома, стройку, сметы, комплектации, продажи, визуализации или коммерческие предложения - отвечай как эксперт строительной компании.
Если речь идет о рендерах дома:
- генерируй только одно изображение
- только фасад спереди
- дом должен быть строго соответствующим параметрам
- без второго ракурса
- без людей
- без текста
- без водяных знаков
- фотореализм
- аккуратный участок
- дневной или мягкий теплый свет
`;
  };
...
Important:
Generate only ONE photorealistic image.
Front facade view only.
No rear view.
No people.
No text.
No watermark.
...
// ============================================================
// РЕНДЕРЫ ЭКОКАРКАС
// Только 1 изображение - фасад спереди
// ============================================================
...
function buildHousePrompt(params) {
  ...
  if (params.windows && params.windows.includes('ламинация')) {
    win += ' with brown wood-grain laminated PVC frames';
  } else {
    win += ' with white PVC frames';
  }
  ...
  const cam = 'strict front facade view, straight-on elevation view from eye level, centered composition, full house in frame, NO rear view, NO side-focused angle';

  if (params.houseType === 'module') {
    const n = params.modulesCount || 1;
    return `Photorealistic exterior photo of ${n} modular prefab timber cabin${n > 1 ? 's connected side by side' : ''}, flat or shallow mono-pitch metal roof, composite wood panel facade, dark accents, large windows, on screw pile foundation. ${size}. ${terrace ? terrace + '. ' : ''}Green lawn, pine trees background. ${cam}. Warm daylight. Architectural visualization, sharp textures, no people, no text, no CGI look, ultra realistic.`;
  }

  if (params.style === 'A-frame') {
    return `Photorealistic exterior photo of an A-frame cabin: ${STYLE['A-frame']}. ${roofMat}. ${size}. ${win}. ${door}. ${terrace ? terrace + '. ' : ''}On concrete foundation, green lawn, mixed trees background, ornamental shrubs. ${cam}. Warm daylight, LED wall lights on facade. Architectural visualization, no people, no text, no CGI.`;
  }
  ...
  const parts = [
    ...
    cam,
    `Warm daylight, soft realistic shadows, clean architectural presentation`,
    `Ultra realistic archviz quality, sharp material textures, no people, no text, no watermarks, no cartoon, no CGI artifacts`
  ].filter(Boolean);

  let prompt = parts.join('. ') + '.';

  if (prompt.length > 3800) {
    prompt = prompt.substring(0, 3800) + '. Ultra realistic architectural visualization.';
  }
...
async function generateHouseRenders(params) {
  const frontPrompt = buildHousePrompt(params);

  console.log('=== DALL-E FRONT ONLY ===');
  console.log(frontPrompt);
  console.log(`Length: ${frontPrompt.length} chars`);

  const frontUrl = await generateImage(frontPrompt);
  const frontBuf = await downloadFile(frontUrl);
